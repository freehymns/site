...
Abc.prototype.set_width = function(s) {
	...
	if (s.play || s.hid) {			// if play symbol
		s.wl = s.wr = 0
		return
	}
...
//fixme: does not work if transposition
	...
	if (s.notes) {				// if note or rest
		...
		if (tie_s) {	// if tie from previous note / grace note
			var b = do_ties(s, tie_s)
			ties_resolved = ties_resolved || b
		}
	}

	if (!ties_resolved && curvoice.tie_s == null) {
		curvoice.tie_s = old_tie_s;
	}
...
//  "mtrC|": {x:0, y:0, c:"\ue08b"},	// cut time (2/2) (unused)
 ...
 "acc-3_2": {x:-3, y:0, c:"\ue281"},	// three-quarter-tones flat
  ...
  ihld: {x:-7, y:-60, c:"\ue4c0"},		// inverted fermata
  brth: {x:0, y:0, c:"\ue4ce"},
  caes: {x:0, y:8, c:"\ue4d1"},
  r00: {x:-1.5, y:0, c:"\ue4e1"},
  r0: {x:-1.5, y:0, c:"\ue4e2"},
...
abc2svg.version="v1.22.23";abc2svg.vdate="2025-04-22"
