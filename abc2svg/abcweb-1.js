...
function dom_loaded() {
	...
	//cem
	abc2svg.get_tune_lst = function() {
		return tune_lst;
	}
	
	abc2svg.get_note_rect = function(n) {
	}
...
function clean_txt(txt) {
	...
	// cem
	abc2svg.init_tune = function(tune) {
		...
		if (!tune_lst[tune]) {
			tune_lst[tune] = abc.tunes[tune]
			abcplay.add(tune_lst[tune][0],
					tune_lst[tune][1],
					tune_lst[tune][3])
		}
	}
	...
	// cem
	/*
	abc2svg.turn_off_notes = function() {
		if (svg_id_playing != null) {
			var svg = document.getElementById(svg_id_playing)

			var rects = svg.getElementsByClassName("abcr")
			for (var rn = 0; rn < rects.length; rn++) {
				if (rects[rn].style.fillOpacity != 0) {
					var rn_i = parseInt(rects[rn].className.baseVal.substring(5).replace("_", ""))
					playconf.onnote(rn_i, false)
				}
			}
			//var last = abc2svg.last_location;
			//var current = abc2svg.current_location;
			//playconf.onnote(last, false);
			//playconf.onnote(current, false);
			if (abc2svg.highlighted_word != null) {
				abc2svg.highlighted_word.style.color = "#000000"
				abc2svg.highlighted_word = null
			}
		}
	}
	*/
	...
	abc2svg.playseq = function(evt) {
		return abc2svg.playsection(evt.target, null)
	}
	
	// function called on click on the music
	abc2svg.playsection = function(start_svg, end_svg, onend, speed) {
		if (playing && onend == null) {		// stop playing
			playing = 0
			if (start_svg == null) {
				playconf.onend = abc2svg.turn_off_notes
			}
			abcplay.stop()
			//playconf.onend()
			//console.log("after stop. playing=" + playing)
			return
		}
		if (!start_svg)
			return
		...
		svg_id_playing = svg.id
		for (var node = svg.parentNode; svg_id_playing == ""; node = node.parentNode) {
			svg_id_playing = node.id
		}
		
		if (onend)
			playconf.onend = function() {
				//console.log("start onend");
				//abc2svg.turn_off_notes();
				var rects = svg.getElementsByClassName("abcr");
				for (var rn = 0; rn < rects.length; rn++) {
					if (rects[rn].style.fillOpacity != 0) {
						var rn_i = parseInt(rects[rn].className.baseVal.substring(5).replace("_", ""));
		...
		if (end_svg) {
			end_s = s
			i = end_svg.getAttribute('class')
			i = i.match(/abcr _(\d+)_/)
			i = i[1]
			//console.log("Looking for " + i)
			while (end_s && end_s.istart != i) {
				//console.log("istart " + end_s.istart)
				end_s = end_s.ts_next
			}
			if (!end_s) {
				alert("play bug: no such end symbol in the tune")
				return
			}
		}
