...
abc2svg.play_next = function(po) {
	...
	while (1) {
		...
		case C.BAR:
			s2 = null
			if (s.rep_p && !abc2svg.skip_repeats) {		// right repeat
				po.repv++
...
//fixme: there may be a tie...
		...
		while (1) {
			if (!s || s == po.s_end || !s.ts_next) {
				if (po.onend) {
					var pause = (d <= .25 ? 25 : 1);
					if (abc2svg.pause_after_tune)
						pause = abc2svg.pause_after_tune
					setTimeout(po.onend,
						(t - now) * 1000 + pause,
						po.repv)
					/*
...
// public methods
	return {
		...
		get_outputs: function(f) {
			if ((abc2svg && abc2svg.no_midi) || !navigator.requestMIDIAccess) {
				f()			// no MIDI
				return
			}
			rf = f
...
function follow(abc, user, playconf) {
	...
	var note_midpt = null;
	var last_scroll_amt = 1;
	var scroll_count = 0;
	var scroll_count_max = 1000;
	
	var scrollnext = function() {
		if (abc2svg.follow_speed != null) {
			if (note_midpt != null) {
				var amt = 0;
				if (note_midpt > window.innerWidth * .9) {
					amt = Math.round(window.innerWidth * .1);
				} else if (note_midpt > window.innerWidth * .8) {
					amt = 8;
				} else if (note_midpt > window.innerWidth * .7) {
					amt = 5;
				} else if (note_midpt > window.innerWidth * .6) {
					amt = Math.min(4, last_scroll_amt + 1);
				} else if (note_midpt > window.innerWidth * .5) {
					amt = Math.min(3, last_scroll_amt + 1);
				} else if (note_midpt > window.innerWidth * .4) {
					amt = 2;
				} else if (note_midpt > window.innerWidth * .3) {
					amt = Math.max(1, last_scroll_amt - 1);
				}
				last_scroll_amt = amt;
				if (!window.no_scroll) {
					window.scrollBy({top: 0, left: amt});
				}
				note_midpt -= amt;
				if (scroll_count == 0) {
					//scroll_count_max = Math.max(1000, window.innerWidth / amt);
					//console.log("setting scroll_count_max = " + scroll_count_max + "\n");
				}
				scroll_count++;
			}
			if (scroll_count < scroll_count_max) {
			    window.setTimeout(scrollnext, abc2svg.follow_speed);
			} else {
...
user.anno_stop = function(type, start, stop, x, y, w, h) {
	...
	playconf.onnote = function(i, on) {
		...
		abc2svg.last_location = abc2svg.current_location;
		abc2svg.current_location = i
		var tune = abc2svg.tune_index_playing;
		var verse = abc2svg.verse_playing;
		//if (!on && abc2svg.highlighted_word != null) {
		//	abc2svg.highlighted_word.style.color = "#000000";
		//}
		if (tune != null && verse != null && abc2svg.codes[tune] != null) {
			var codeword = abc2svg.codes[tune]["abcr" + i];
			if (codeword != null && abc2svg.words[tune][verse] != null) {
				var current_word = abc2svg.words[tune][verse][codeword];
				if (current_word == null && abc2svg.words[tune][0] != null) {
					current_word = abc2svg.words[tune][0][codeword];
				}
				//if (current_word == null) {
				//	console.log (i + " " + codeword + " " + on + " null");
				//}
				//if (current_word == null && on) {
				//	current_word = abc2svg.highlighted_word;
				//}
				if (current_word != null) {
					if (on) {
						//console.log(i + " " + codeword + " on");
						current_word.style.color = "#c00000";
					} else {
						//console.log(i + " " + codeword + " off");
						current_word.style.color = "#000000";
					}
					abc2svg.highlighted_word = current_word;
				}
			}
		}
		...
		e.style.fillOpacity = on ? 0.4 : 0
			...
			if (on && !window.no_scroll) {	
				...
				scroll_count = 0;
				note_midpt = b.left + (b.right - b.left) / 2
				
				var breakpt = (abc2svg.follow_speed == null ? .8 : .95)

				// normal
				if (b.top < 0
				 || b.bottom > window.innerHeight * breakpt)
					y = b.top - window.innerHeight * .3

				// single line
				if (b.left < 0
				 || b.left > window.innerWidth * breakpt)
